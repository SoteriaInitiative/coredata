...
def _resolve_schema_path() -> str:
    """Resolve path to XML_Schema.xsd supporting multiple layouts.

    Order:
    - GOAML_SCHEMA_PATH env var
    - Package resources: XML_Schema.xsd at package root or standard/XML_Schema.xsd
    - Common relative paths when running from a repo checkout
    """
    # 1) Explicit override via environment
    env_path = os.getenv('GOAML_SCHEMA_PATH')
    ...
    candidates = [
        ('XML_Schema.xsd',),
        ('standard', 'XML_Schema.xsd'),
    ]
    pkg_candidates = []
    if __package__:
        pkg_candidates.append(__package__.split('.')[0])
        pkg_candidates.append(__package__)
    pkg_candidates.extend(['src'])
    for pkg in pkg_candidates:
        try:
            for parts in candidates:
                res = importlib_resources.files(pkg).joinpath(*parts)
                try:
    ...
    # 3) Fallback to common relative filesystem paths
    here = os.path.dirname(__file__)
    rels = [
        os.path.join(here, 'XML_Schema.xsd'),
        os.path.join(here, 'standard', 'XML_Schema.xsd'),
        os.path.join(here, '..', 'standard', 'XML_Schema.xsd'),
    ]
    for p in rels:
        if os.path.exists(p):
            return p
    raise FileNotFoundError('Unable to locate XML_Schema.xsd; set GOAML_SCHEMA_PATH to the schema file.')
...
def _build_account(parent, account, currency_code_local, day, tag):
    ...
    if account.get('party_type') == 'entity':
        ...
        etree.SubElement(are_holder, 'account_entity_relation').text = 'ACCCO'
        ent_holder = etree.SubElement(are_holder, 'entity')
        _build_entity(ent_holder, account.get('name', 'Unknown'), account.get('legal_form', 'AG'), addr)
...
def generate_reports(args):
    ...
    print(f"Storage bucket name: {gs_utils.storage_client.bucket(gs_utils.BUCKET_NAME)}")
    print(f"Storage bucket folder: {folder}")
